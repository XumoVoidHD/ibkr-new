...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('logs/strategy_log.txt', mode='w'),
...
class Strategy:
    ...
    async def main(self):
        self.logger.info("1. Testing connection...")
        await self.broker.connect()
        self.logger.info(f"Connection status: {self.broker.is_connected()}")
        ...
        while True:
            ...
            if (start_time <= current_time <= closing_time) or self.testing:
                ...
                self.logger.info(f"CLOSEST CURRENT PRICE: {closest_strike}")

                self.otm_closest_call = closest_strike + (credentials.OTM_CALL_HEDGE * 5)
                self.logger.info(f"CALL HEDGE STRIKE PRICE: {self.otm_closest_call}")

                self.otm_closest_put = closest_strike - (credentials.OTM_PUT_HEDGE * 5)
                self.logger.info(f"PUT HEDGE STRIKE PRICE: {self.otm_closest_put}")

                self.call_target_price = closest_strike
                if credentials.ATM_CALL > 0:
                    self.call_target_price += 5 * credentials.ATM_CALL
                self.logger.info(f"CALL POSITION STRIKE PRICE: {self.call_target_price}")

                self.put_target_price = closest_strike
                if credentials.ATM_CALL > 0:
                    self.put_target_price -= 5 * credentials.ATM_CALL
                self.logger.info(f"PUT POSITION STRIKE PRICE: {self.put_target_price}")
                ...
                self.logger.info("Market hasn't opened yet")
    ...
    async def place_hedge_orders(self, call, put):
        if call:
            ...
            try:
                ...
                self.logger.error(f"Error placing call hedge order: {str(e)}")
        ...
        if put:
            ...
            try:
                ...
                self.logger.error(f"Error placing put hedge order: {str(e)}")
    ...
    async def close_open_hedges(self, close_put=False, close_call=False):
        if close_call:
            ...
            try:
                ...
                self.logger.error(f"Error closing call hedge: {str(e)}")
        ...
        if close_put:
            ...
            try:
                ...
                self.logger.error(f"Error closing put hedge: {str(e)}")
    ...
    async def place_atm_call_order(self):
        ...
        self.logger.info(f'last price is {premium_price["last"]}')
        ...
        while self.should_continue:
            if self.call_order_placed:
                ...
                if temp_percentage <= 0:
                    self.logger.info(f"Call trailing sl is at {temp_percentage}")
                    continue
                ...
                if not self.call_rentry < credentials.number_of_re_entry:
                    self.logger.info("Call re-entry limit reached")
                    return
    ...
    async def put_check(self):
        ...
        while self.should_continue:
            if self.put_order_placed:
                ...
                if temp_percentage <= 0:
                    self.logger.info(f"Put trailing sl is at {temp_percentage}")
                    continue
                ...
                if not self.put_rentry < credentials.number_of_re_entry:
                    self.logger.info("Put re-entry limit reached")
                    return
